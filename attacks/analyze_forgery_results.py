...
# Calculate means with checks for empty DataFrames
original_detection_rate = df['original_watermark_detected'].mean() if not df.empty else 0
forged_detection_rate = df['forged_watermark_detected'].mean() if not df.empty else 0

print(f"Original Watermark Detection Rate: {original_detection_rate:.2%}")
print(f"Forged Watermark Detection Rate: {forged_detection_rate:.2%}")

# Create a bar chart of success rates
plt.figure(figsize=(10, 6))
labels = ['Original Images', 'Forged Images']
values = [original_detection_rate, forged_detection_rate]
colors = ['blue', 'red']
...
with open(report_path, 'w') as f:
    ...
    f.write(f"- Original Watermark Detection Rate: {original_detection_rate:.2%}\n")
    f.write(f"- Forged Watermark Detection Rate: {forged_detection_rate:.2%}\n\n")
