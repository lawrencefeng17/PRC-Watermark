...
for t_val in t_values:
    ...
    for p in tqdm(error_rates, desc=f"Error rate sweep (t={t_val})"):
        detections = 0
        num_flips = int(np.round(n * p))
        for _ in range(num_trials):
            # Corrupt codeword: flip the first num_flips bits
            corrupted = x_prc_binary.copy()
            corrupted[:num_flips] = (corrupted[:num_flips] ^ 1) % 2
...
plt.title(f'PRC Substitution Error Tolerance (n={n}, trials={num_trials}, t=3-15, first np bits flipped)')
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.savefig('prc_error_tolerance_firstbits_multiple_t.png')
plt.close()

print("\nExperiment complete. Plot saved to prc_error_tolerance_firstbits_multiple_t.png")
...
for n_val in n_values:
    ...
    for p_err in tqdm(error_rates, desc=f"Error rate sweep (n={n_val}, t={t_fixed})"):
        detections_n = 0
        num_flips_n = int(np.round(n_val * p_err))
        for _ in range(num_trials):
            # Corrupt codeword: flip the first num_flips_n bits
            corrupted_n = x_prc_binary_n.copy()
            corrupted_n[:num_flips_n] = (corrupted_n[:num_flips_n] ^ 1) % 2
...
plt.xlabel('Bit Error Rate ($p_{emb}$)')
plt.ylabel('Detection Rate')
plt.title(f'PRC Error Tolerance (t={t_fixed}, trials={num_trials}, n varied, first np bits flipped)')
plt.legend()
plt.grid(True)
plt.tight_layout()
new_plot_filename = f'prc_error_tolerance_firstbits_multiple_n_t_fixed_{t_fixed}.png'
